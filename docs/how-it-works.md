...
For a real investigation, see the [Netclaw case study](netclaw-case-study.md): follow catalog entries
into scattered correction handlers, an Auto-approval ordering difference, and separate process starts.
It includes an agent prompt, manual commands, and the resulting consolidation direction.
